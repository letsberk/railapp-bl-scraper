    ...
    if token and token != "cookie":
        ...
        try:
            ...
            if token_for_header:
                ...
                print(f"  User-Token-Data: {hdrs['User-Token-Data'][:100]}")
...
async def select_role(client: httpx.AsyncClient, token: str) -> str:
    """Wählt die erste verfügbare Rolle/Filterprofil aus → aktualisiert Token mit currentSelectedRole"""
    import json as _j
    try:
        td = _j.loads(token)
        jwt     = td.get("jwt","")
        ...
        hdrs = {
            ...
            "Referer":          "https://betrieblive.noncd.db.de/aktuell",
            "X-Version":        "2026.4.1",
            "User-Token-Data":  decode_jwt_payload(role_tok or jwt),
        }
        if role_tok:
            hdrs["Cookie"] = f"ROLE_SELECT_TOKEN={role_tok}"

        # Rollen abrufen
        r = await client.get("/api/v1/roles?limit=0", headers=hdrs, timeout=10)
        print(f"  GET roles → {r.status_code}")
        if r.status_code != 200:
            return token

        roles_data = r.json()
        roles = roles_data if isinstance(roles_data, list) else                 roles_data.get("roles") or roles_data.get("data") or []
        print(f"  Verfügbare Rollen: {len(roles)}")

        if not roles:
            return token

        # Erste Rolle auswählen
        role_id = roles[0].get("_id") or roles[0].get("id","")
        print(f"  Wähle Rolle: {role_id}")

        # Rolle setzen
        for ep in [
            f"/api/v1/authentications/{uid}/currentSelectedRole",
            f"/api/v1/users/{uid}/currentSelectedRole",
            "/api/v1/authentications/currentSelectedRole",
        ]:
            try:
                pr = await client.put(ep,
                    json={"currentSelectedRole": role_id},
                    headers={**hdrs, "Content-Type": "application/json"},
                    timeout=10)
                print(f"  PUT {ep} → {pr.status_code}")
                if pr.status_code in (200, 201, 204):
                    # Neues Token aus Response holen
                    if pr.status_code != 204:
                        new_data = pr.json() if "json" in pr.headers.get("content-type","") else {}
                        new_jwt  = new_data.get("token","")
                    new_role = pr.cookies.get("ROLE_SELECT_TOKEN", role_tok)
                    print(f"  ✅ Rolle gesetzt")
                    return _j.dumps({
                        "jwt":       new_jwt or jwt,
                        "userId":    uid,
                        "roleToken": new_role,
