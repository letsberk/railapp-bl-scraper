...
def decode_jwt_payload(jwt_str: str) -> str:
    ...
    import base64, json
    try:
        # Token ist base64(JWT) - erst äußeres standard-base64 dekodieren
        raw = base64.b64decode(jwt_str + "==").decode("utf-8", errors="ignore")
        if "." in raw:
            jwt_str = raw  # War doppelt kodiert

        # JWT Payload (mittlerer Teil) - JWT nutzt URL-safe base64
        parts = jwt_str.split(".")
        if len(parts) < 2:
            return jwt_str

        payload_b64 = parts[1]
        # URL-safe base64 → standard (padding ergänzen)
...
async def fetch(client: httpx.AsyncClient, token: str) -> list:
    ...
    if token and token != "cookie":
        try:
            ...
            access_tok = td.get("accessToken", jwt)
            override   = td.get("tokenOverride","")
            user_token = override or decode_jwt_payload(role_tok or jwt)
        except Exception as e:
            print(f"  Token-Parse: {e}")
            user_token = token
            access_tok = token
    ...
    hdrs = {
        ...
        "X-Version":        "2026.4.1",
        "X-Trigger-Socket": "railapp-bot",
        "User-Agent":       "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        "User-Token-Data":  user_token,
    }
    # ACCESS_TOKEN als Cookie (wie Browser) - KEIN Authorization Bearer
    fetch_cookies = {"ACCESS_TOKEN": access_tok} if access_tok else {}
    print(f"  User-Token-Data: {user_token[:80]}")
    print(f"  ACCESS_TOKEN cookie: {'gesetzt' if access_tok else 'fehlt'}")
    ...
    try:
        r = await client.get(ep, headers=hdrs, cookies=fetch_cookies, timeout=20)
...
async def fetch_chat(client: httpx.AsyncClient, token: str, incident_id: str) -> list:
    ...
    if token and token != "cookie":
        ...
        try:
            ...
            # ROLE_SELECT_TOKEN enthält currentSelectedRole → für User-Token-Data nutzen
            # tokenOverride hat currentSelectedRole → direkt nutzen
            override = td.get("tokenOverride","")
            if override:
                hdrs["User-Token-Data"] = override
            else:
                token_for_header = role_tok or jwt
                if token_for_header:
                    hdrs["User-Token-Data"] = decode_jwt_payload(token_for_header)
            print(f"  User-Token-Data: {hdrs.get('User-Token-Data','–')[:100]}")

            # Cookies werden automatisch von httpx verwaltet (Login-Session)
            # NICHT manuell setzen - überschreibt sonst den TS-Security-Cookie
        except Exception as e:
            print(f"  Token-Decode Fehler: {e}")
    ep = f"{BL_API_BASE}/api/v1/incidents/{incident_id}/messages/"
    try:
        r = await client.get(ep, headers=hdrs, cookies=chat_cookies, timeout=10)
        if r.status_code == 200:
            data = r.json()
...
# ── ROLLE AUSWÄHLEN ────────────────────────────────────────
# Rolle-ID aus Browser-DevTools (kann als BL_ROLE_ID Variable gesetzt werden)
BL_ROLE_ID = os.environ.get("BL_ROLE_ID", "5a12ed59fc94890009ad6c7d")

async def select_role(client: httpx.AsyncClient, token: str) -> str:
    """Wählt Rolle via API - exakt wie Browser"""
    import json as _j
    ...
    try:
        ...
        base_hdrs = {
            "Accept":      "application/json, text/plain, */*",
            "X-Version":   "2026.4.1",
            "Referer":     f"{BL_BASE}/aktuell",
            "User-Agent":  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        }

        # Schritt 1: GET authorizations (wie Browser)
        r1 = await client.get("/api/v1/authorizations", headers=base_hdrs, timeout=10)
        print(f"  GET authorizations → {r1.status_code} ({len(r1.content)}B)")

        # Schritt 2: PUT role selection
        for ep in [
            f"/api/v1/users/{uid}/currentSelectedRole",
            f"/api/v1/authentications/{uid}/currentSelectedRole",
            f"/api/v1/roles/{BL_ROLE_ID}/select",
            "/api/v1/currentSelectedRole",
        ]:
            try:
                r2 = await client.put(ep,
                    json={"currentSelectedRole": BL_ROLE_ID},
                    headers={**base_hdrs, "Content-Type":"application/json"},
                    timeout=10)
                print(f"  PUT {ep} → {r2.status_code}")
                if r2.status_code in (200,201,204):
                    new_role = r2.cookies.get("ROLE_SELECT_TOKEN", role_tok)
                    print("  ✅ Rolle via API gesetzt")
                    break
            except Exception:
                pass

        # Schritt 3: Token-Override mit expirationDate (30 Tage) wie Browser
        expiry = (datetime.utcnow() + timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%S.000Z")
        payload_str = decode_jwt_payload(role_tok or jwt)
        try:
            payload = _j.loads(payload_str)
        except Exception:
            payload = {"_id": uid}

        # Browser-kompatible Felder setzen
        payload.pop("exp", None)  # exp entfernen
        payload["currentSelectedRole"] = BL_ROLE_ID
        payload["expirationDate"]      = expiry  # 30 Tage wie Browser
        override = _j.dumps(payload)

        print(f"  Override: {override[:120]}")
        return _j.dumps({
            "jwt":           jwt,
            "userId":        uid,
            "roleToken":     role_tok,
...
async def main():
    ...
    async with httpx.AsyncClient(base_url=BL_BASE, follow_redirects=True) as client:
        ...
        # Rolle auswählen (setzt currentSelectedRole im Token)
        token = await select_role(client, token) or token

        login_time = datetime.now()
        while True:
            try:
                # Token alle 4 Minuten erneuern (läuft nach 5 Min ab)
                if (datetime.now() - login_time).seconds >= 240:
