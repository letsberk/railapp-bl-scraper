...
def build_auth_headers(token: str) -> dict:
    """Erzeugt die Auth-Header für Betrieb Live."""
    hdrs = {
        "Accept":      "application/json, text/plain, */*",
        "Referer":     "https://betrieblive.noncd.db.de/aktuell",
        "Origin":      "https://betrieblive.noncd.db.de",
        "X-Version":    "2026.4.1",
        "Dnt":         "1",
    ...
    if token and token != "cookie":
        ...
        try:
            ...
            # Wichtig: Bearer Token mitsenden.
            # Viele APIs akzeptieren den Session-Cookie allein nicht zuverlässig.
            bearer = role_tok or jwt
            if bearer:
                hdrs["Authorization"] = f"Bearer {bearer}"

            # User-Token-Data möglichst kompakt senden.
            if override:
                try:
                    hdrs["User-Token-Data"] = _j.dumps(_j.loads(override), separators=(",", ":"))
                except Exception:
...
async def fetch(client: httpx.AsyncClient, token: str) -> list:
    hdrs = build_auth_headers(token)

    # Daten kommen von noncd.db.de
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"  ⚠ incidents: {r.status_code} – {r.text[:300]}")
...
async def select_role(client: httpx.AsyncClient, token: str) -> str:
    """Fügt currentSelectedRole zum Token hinzu"""
    import json as _j
    try:
        td       = _j.loads(token)
        jwt      = td.get("jwt","")
        ...
        # Payload dekodieren und currentSelectedRole hinzufügen
        ...
        print(f"  currentSelectedRole: {BL_ROLE_ID}")
        print(f"  Token-Override: {override[:100]}")
