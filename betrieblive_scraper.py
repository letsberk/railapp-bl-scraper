...
import sys
os.environ["PYTHONUNBUFFERED"] = "1"  # Sofort flush
...
async def main():
    print("main() gestartet", flush=True)
    async with async_playwright() as pw:
        ...
        print(f"Navigiere zu {BL_URL}/#/login", flush=True)
        await page.goto(f"{BL_URL}/#/login", wait_until="networkidle", timeout=30000)
        await asyncio.sleep(3)
        print(f"URL: {page.url}", flush=True)

        # Alle Input-Felder loggen
        inputs = await page.query_selector_all("input")
        print(f"Input-Felder gefunden: {len(inputs)}", flush=True)
        for inp in inputs:
            t = await inp.get_attribute("type")
            n = await inp.get_attribute("name")
            p = await inp.get_attribute("placeholder")
            print(f"  input type={t} name={n} placeholder={p}", flush=True)

        # Username eingeben
        filled_user = False
        for sel in ['input[type="text"]','input[name="name"]','#name','input']:
            try:
                el = await page.wait_for_selector(sel, timeout=2000)
                if el:
                    await el.fill(BL_USERNAME)
                    print(f"Username via '{sel}'", flush=True)
                    filled_user = True
                    break
            except: pass
        ...
        for sel in ['input[type="password"]','#password']:
            try:
                el = await page.wait_for_selector(sel, timeout=2000)
                if el:
                    await el.fill(BL_PASSWORD)
                    print(f"Passwort via '{sel}'", flush=True)
                    filled_pw = True
                    break
            except: pass

        print(f"Felder ausgefüllt: user={filled_user} pw={filled_pw}", flush=True)

        # Submit
        for sel in ['button[type="submit"]','button:has-text("Anmelden")','button:has-text("Login")','form button']:
            try:
        ...
        print(f"URL nach Login: {page.url}", flush=True)

        if "login" in page.url.lower():
            print("Login fehlgeschlagen - Seiteninhalt:", flush=True)
            txt = await page.inner_text("body")
            print(txt[:300], flush=True)
            await browser.close()
            return
        print(f"Eingeloggt: {page.url}", flush=True)
