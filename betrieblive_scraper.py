...
async def login(client: httpx.AsyncClient) -> str:
    endpoints = [
        ("/api/auth/login",   {"username": BL_USERNAME, "password": BL_PASSWORD}),
        ("/api/auth/login",   {"email":    BL_USERNAME, "password": BL_PASSWORD}),
        ("/api/login",        {"username": BL_USERNAME, "password": BL_PASSWORD}),
        ("/api/v1/auth/login",{"username": BL_USERNAME, "password": BL_PASSWORD}),
        ("/auth/login",       {"username": BL_USERNAME, "password": BL_PASSWORD}),
    ]
    for ep, body in endpoints:
        try:
            ...
            if r.status_code in (200, 201):
                ...
                if token:
                    print(f"✅ Login via {ep}")
                    return str(token)
                if r.cookies:
                    print(f"✅ Cookie-Login via {ep}")
                    return "cookie"
        except Exception:
            pass
    ...
    try:
        r = await client.post("/login",
            data={"username": BL_USERNAME, "password": BL_PASSWORD},
