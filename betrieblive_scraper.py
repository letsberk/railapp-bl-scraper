...
async def login(client: httpx.AsyncClient) -> str:
    ...
    # Exaktes Format aus Browser-DevTools:
    # {"name": "Knotendisponent FF", "password": "...", "sameSiteCookiePolicy": "lax"}
    bodies = [
        {"name": BL_USERNAME, "password": BL_PASSWORD, "sameSiteCookiePolicy": "lax"},
        {"name": BL_USERNAME, "password": BL_PASSWORD},
    ]
    for body in bodies:
        try:
            ...
            if r.status_code in (200, 201):
                ...
                print(f"  POST Response: {str(data)[:200]}")
                ...
                # BetriebLive: nach POST login, GET authentications aufrufen
                # um vollständige Token-Daten mit currentSelectedRole zu holen
                ...
                try:
                    ...
                    if get_r.status_code == 200:
                        get_data = get_r.json()
                        token_str = _json.dumps(get_data)
                        print(f"✅ Login OK – Token: {list(get_data.keys()) if isinstance(get_data, dict) else type(get_data).__name__}")
                        return token_str
                except Exception as ge:
                    print(f"  GET auth: {ge}")
                # Fallback: POST-Response direkt
                token_data = _json.dumps(data) if data else ""
                if token_data and len(token_data) > 10:
                    print(f"✅ Login OK – POST Token")
                    return token_data
                if token:
                    return str(token)
                print(f"✅ Login OK – cookie")
                return "cookie"
...
async def fetch(client: httpx.AsyncClient, token: str) -> list:
    # Exakte Header wie im Browser (aus DevTools Screenshot)
    hdrs = {
        "Accept":           "application/json, text/plain, */*",
