    ...
    for body in bodies:
        try:
            ...
            if r.status_code in (200, 201):
                ...
                # BetriebLive nutzt User-Token-Data Header (JSON-Objekt)
                # Das gesamte Response-Objekt wird als Token zurückgegeben
                import json as _json
                token_data = _json.dumps(data) if data else ""
                if token_data and len(token_data) > 10:
                    print(f"✅ Login OK – User-Token-Data gesetzt")
                    return token_data
                if token:
                    return str(token)
                print(f"✅ Login OK – {list(data.keys()) if data else 'leer'}")
                return "cookie"
...
async def fetch(client: httpx.AsyncClient, token: str) -> list:
    # BetriebLive nutzt User-Token-Data Header (JSON-String)
    hdrs = {"Accept": "application/json"}
    if token and token != "cookie":
        hdrs["User-Token-Data"] = token  # Das ist der korrekte Auth-Header
